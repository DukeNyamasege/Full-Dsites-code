...
const AutoTrades = observer(() => {
        ...
        for (const market of selectedMarketsRef.current) {
            ...
            if (!subscriptionsRef.current[market.symbol]) {
                try {
                    ...
                    const sub = safeSubscribe(
                        ...
                        (data: any) => {
                            ...
                            if (data?.error) {
                                ...
                                requestDataRecoveryRef.current(`tick stream error:${market.symbol}`);
                                return;
                            }
                        ...
                        (streamError: unknown) => {
                            ...
                            requestDataRecoveryRef.current(`tick stream failure:${market.symbol}`);
                        }
                    );
                    ...
                    requestDataRecoveryRef.current(`tick subscribe failure:${market.symbol}`);
                }
            }
            ...
            if (!candleSubscriptionsRef.current[market.symbol]) {
                try {
                    ...
                    const sub = safeSubscribe(
                        ...
                        (data: any) => {
                            ...
                            if (data?.error) {
                                ...
                                requestDataRecoveryRef.current(`candle stream error:${market.symbol}`);
                                return;
                            }
                            const candle =
                                data?.ohlc ??
                        ...
                        (streamError: unknown) => {
                            ...
                            requestDataRecoveryRef.current(`candle stream failure:${market.symbol}`);
                        }
                    );
                    ...
                    requestDataRecoveryRef.current(`candle subscribe failure:${market.symbol}`);
                }
            }
        }
    ...
    const restartSubscriptions = useCallback((forceReconnect = false) => {
        ...
        window.setTimeout(() => {
            ...
            recoverTradingDataConnection({ forceReconnect, source: 'AutoTrades' })
    ...
    requestDataRecoveryRef.current = (reason: string) => {
        ...
        recoveryTimerRef.current = window.setTimeout(() => {
            ...
            console.warn(`[AutoTrades] Recovering market data after ${reason}.`);
    ...
    useEffect(() => {
        ...
        dataSilenceIntervalRef.current = window.setInterval(() => {
            ...
            if (has_selected_markets && isRecoveringDataRef.current) {
