...
const DATA_INTERRUPTION_RESTART_MS = 250;
const DATA_RECOVERY_ESCALATION_MS = 30000;
const CONTRACT_POLL_MAX_RETRIES = 50;
...
const Combo = observer(() => {
    ...
    const requestDataRecoveryRef = useRef<(reason: string) => void>(() => {});
    ...
    const pollContract = useCallback(
        (contractId: number): Promise<Record<string, any>> =>
            new Promise(resolve => {
                let retryCount = 0;
                const check = async () => {
                    if (unmountedRef.current || retryCount >= CONTRACT_POLL_MAX_RETRIES) {
                    ...
                    try {
                        ...
                        if (!isExpectedStreamInterruption(error)) {
                            console.warn('[Combo] Contract poll error:', error);
                        }
                        resolve({ profit: 0, is_sold: true });
                    }
                };
    ...
    const subscribe = useCallback(
        (row: ComboRow) => {
            ...
            try {
                ...
                const sub = safeSubscribe(
                    ...
                    (data: any) => {
                        ...
                        if (data?.error) {
                            ...
                            requestDataRecoveryRef.current(`tick stream error:${row.symbol}`);
                            return;
                        }
                    ...
                    (streamError: unknown) => {
                        ...
                        requestDataRecoveryRef.current(`tick stream failure:${row.symbol}`);
                    }
                );
                ...
                requestDataRecoveryRef.current(`tick subscribe failure:${row.symbol}`);
            }
        },
    ...
    const restartSubscriptions = useCallback((forceReconnect = false) => {
        ...
        window.setTimeout(() => {
            ...
            recoverTradingDataConnection({ forceReconnect, source: 'Combo' })
                .then(() => subscribeAll())
                .catch(err => {
                    console.error('[Combo] Data restart failed:', err);
                })
                .finally(() => {
                    restartInFlightRef.current = false;
                    lastTickAtRef.current = Date.now();
    ...
    requestDataRecoveryRef.current = (reason: string) => {
        if (!show_combo || unmountedRef.current) return;

        const now = Date.now();
        if (now - recoveryWindowStartedAtRef.current > DATA_RECOVERY_ESCALATION_MS) {
            recoveryWindowStartedAtRef.current = now;
            recoveryAttemptCountRef.current = 0;
        }
        recoveryAttemptCountRef.current++;

        if (recoveryTimerRef.current || restartInFlightRef.current) return;

        recoveryTimerRef.current = window.setTimeout(() => {
            recoveryTimerRef.current = null;
            const should_force_reconnect = recoveryAttemptCountRef.current > 1;
            console.warn(`[Combo] Recovering market data after ${reason}.`);
            restartSubscriptions(should_force_reconnect);
        }, DATA_INTERRUPTION_RESTART_MS);
    };
                ...
                if (isConnected) {
                    ...
                    setTimeout(() => {
                        ...
                        try {
                            ...
                            const sub = safeSubscribe(
                                ...
                                (data: any) => {
                                    ...
                                    if (data?.error) {
                                        ...
                                        requestDataRecoveryRef.current(`tick stream error:${value}`);
                                        return;
                                    }
                                ...
                                (streamError: unknown) => {
                                    ...
                                    requestDataRecoveryRef.current(`tick stream failure:${value}`);
                                }
                            );
                            ...
                            requestDataRecoveryRef.current(`tick subscribe failure:${value}`);
                        }
                    }, 100);
                }
            }
    ...
    useEffect(() => {
        ...
        const id = window.setInterval(() => {
            ...
            if (rowsRef.current.length > 0 && isRecoveringDataRef.current) {
                requestDataRecoveryRef.current('watchdog detected interrupted stream');
                return;
            }
